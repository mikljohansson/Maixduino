...
static uint8_t sensor_default_regs[][2] = {
	{0xfe , 0x80},  //reset
	{0xfe , 0x80},  //reset
...
{    
    for (uint16_t index = 0; sensor_default_regs[index][0]; index++)
    {
        uint8_t reg_data;
        cambus_writeb(GC0328_ADDR, sensor_default_regs[index][0], sensor_default_regs[index][1]);
        cambus_readb(GC0328_ADDR,sensor_default_regs[index][0],&reg_data);
